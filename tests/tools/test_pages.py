...
class TestUpdatePageSettings:
    ...
    async def test_publish_page(
        ...
        result = await update_page_settings(
            course_identifier="67619", page_url_or_id="test-page", published=True
        )
    ...
    async def test_unpublish_page(
        ...
        update_page_settings = get_tool_function("update_page_settings")
        result = await update_page_settings(
            course_identifier="67619", page_url_or_id="test-page", published=False
        )
    ...
    async def test_set_front_page(
        ...
        update_page_settings = get_tool_function("update_page_settings")
        result = await update_page_settings(
            course_identifier="67619", page_url_or_id="home-page", front_page=True
        )
    ...
    @pytest.mark.asyncio
    async def test_change_editing_roles(
        self, mock_canvas_request, mock_course_id, mock_course_code
    ):
    ...
    @pytest.mark.asyncio
    async def test_no_changes_specified(
        self, mock_canvas_request, mock_course_id, mock_course_code
    ):
        """Test error when no settings are specified to update."""
        update_page_settings = get_tool_function("update_page_settings")
        result = await update_page_settings(
            course_identifier="67619", page_url_or_id="test-page"
        )
    ...
    async def test_api_error_handling(
        ...
        mock_canvas_request.return_value = {"error": "Page not found"}

        update_page_settings = get_tool_function("update_page_settings")
        result = await update_page_settings(
            course_identifier="67619", page_url_or_id="nonexistent-page", published=True
        )

        assert "error" in result.lower()

    @pytest.mark.asyncio
    async def test_cannot_unpublish_front_page(
        self, mock_canvas_request, mock_course_id, mock_course_code
    ):
        """Test that unpublishing front page returns appropriate error."""
        mock_canvas_request.return_value = {"error": "Cannot unpublish the front page"}

        update_page_settings = get_tool_function("update_page_settings")
        result = await update_page_settings(
            course_identifier="67619", page_url_or_id="front-page", published=False
        )

        assert "error" in result.lower() or "cannot" in result.lower()

    @pytest.mark.asyncio
    async def test_multiple_settings_update(
        self, mock_canvas_request, mock_course_id, mock_course_code
    ):
...
class TestBulkUpdatePages:
    ...
    @pytest.mark.asyncio
    async def test_bulk_publish_pages(
        self, mock_canvas_request, mock_course_id, mock_course_code
    ):
        ...
        result = await bulk_update_pages(
            course_identifier="67619", page_urls="page-1,page-2,page-3", published=True
        )
    ...
    @pytest.mark.asyncio
    async def test_bulk_unpublish_pages(
        self, mock_canvas_request, mock_course_id, mock_course_code
    ):
        ...
        bulk_update_pages = get_tool_function("bulk_update_pages")
        result = await bulk_update_pages(
            course_identifier="67619", page_urls="page-1,page-2", published=False
        )
    ...
    async def test_bulk_update_empty_list(
        ...
        bulk_update_pages = get_tool_function("bulk_update_pages")
        result = await bulk_update_pages(
            course_identifier="67619", page_urls="", published=True
        )

        assert (
            "no pages" in result.lower()
            or "empty" in result.lower()
            or "specify" in result.lower()
        )
        mock_canvas_request.assert_not_called()

    @pytest.mark.asyncio
    async def test_bulk_update_no_settings(
        self, mock_canvas_request, mock_course_id, mock_course_code
    ):
        """Test error when no settings specified for bulk update."""
        bulk_update_pages = get_tool_function("bulk_update_pages")
        result = await bulk_update_pages(
            course_identifier="67619", page_urls="page-1,page-2"
        )
...
class TestInputValidation:
    ...
    @pytest.mark.asyncio
    async def test_invalid_editing_roles(
        self, mock_canvas_request, mock_course_id, mock_course_code
    ):
        """Test validation of editing_roles parameter."""
        mock_canvas_request.return_value = {"error": "Invalid editing_roles"}
        ...
        assert (
            ...
            or mock_canvas_request.called
        )

    @pytest.mark.asyncio
    async def test_page_url_with_special_characters(
        self, mock_canvas_request, mock_course_id, mock_course_code
    ):
