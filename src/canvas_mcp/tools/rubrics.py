...
from ..core.dates import truncate_text
...
def validate_rubric_criteria(criteria_json: str) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            error_msg += (
                f"First 200 characters of original: {repr(criteria_json[:200])}\n"
            )
            error_msg += (
                f"First 200 characters of cleaned: {repr(cleaned_json[:200])}\n"
            )
            if len(cleaned_json) > 200:
                error_msg += (
                    f"Last 100 characters of cleaned: {repr(cleaned_json[-100:])}"
                )
    ...
    for criterion_key, criterion_data in criteria.items():
        ...
        try:
            ...
            raise ValueError(
                f"Criterion {criterion_key} points must be a valid number"
            ) from err
            ...
            if isinstance(ratings, dict):
                ...
                for rating_key, rating_data in ratings.items():
                    ...
                    try:
                        ...
                        raise ValueError(
                            f"Rating {rating_key} points must be a valid number"
                        ) from err
                ...
                for i, rating_data in enumerate(ratings):
                    ...
                    try:
                        ...
                        raise ValueError(
                            f"Rating {i} points must be a valid number"
                        ) from err
...
def register_rubric_tools(mcp: FastMCP) -> None:
    ...
    @mcp.tool()
    @validate_params
    async def list_assignment_rubrics(
        ...
        result = (
            f"Rubric for Assignment '{assignment_name}' in Course {course_display}:\n\n"
        )
        ...
        if rubric_settings:
            ...
            result += (
                f"  Used for Grading: {'Yes' if use_rubric_for_grading else 'No'}\n"
            )
            result += (
                f"  Points Possible: {rubric_settings.get('points_possible', 'N/A')}\n"
            )
    ...
    @mcp.tool()
    @validate_params
    async def get_assignment_rubric_details(
        course_identifier: str | int, assignment_id: str | int
    ) -> str:
    ...
    async def get_rubric_details(
        course_identifier: str | int, rubric_id: str | int
    ) -> str:
    ...
    async def list_all_rubrics(
        course_identifier: str | int, include_criteria: bool = True
    ) -> str:
                ...
                for j, criterion in enumerate(data, 1):
                    ...
                    if ratings:
                        ...
                        for rating in sorted_ratings:
                            ...
                            if (
                                rating_long_desc
                                and rating_long_desc != rating_description
                            ):
    ...
    async def create_rubric(
        ...
        try:
            ...
            if isinstance(criteria, str):
                ...
                return "Error: criteria must be a JSON string or dictionary object"  # type: ignore[unreachable]
        ...
        # Build request data
        request_data = {"rubric": rubric_data}
        ...
        response = await make_canvas_request(
            "post", f"/courses/{course_id}/rubrics", data=request_data
        )
    ...
    async def update_rubric(
        ...
        skip_updating_points_possible: bool = False,
    ) -> str:
    ...
    @mcp.tool()
    @validate_params
    async def delete_rubric(course_identifier: str | int, rubric_id: str | int) -> str:
        ...
        rubric_response = await make_canvas_request(
            "get", f"/courses/{course_id}/rubrics/{rubric_id_str}"
        )
        ...
        # Delete the rubric
        response = await make_canvas_request(
            "delete", f"/courses/{course_id}/rubrics/{rubric_id_str}"
        )
    ...
    @mcp.tool()
    @validate_params
    async def associate_rubric_with_assignment(
        course_identifier: str | int,
        rubric_id: str | int,
        ...
        # Make the API request
        response = await make_canvas_request(
            "put", f"/courses/{course_id}/rubrics/{rubric_id_str}", data=request_data
        )
